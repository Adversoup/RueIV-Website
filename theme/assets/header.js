class BasicHeader extends HTMLElement {
  ...
  setupHoverDetection() {
    this.addEventListener('mouseenter', () => {
      this.headerSection.classList.add('header-hovered');
    });
    this.addEventListener('mouseleave', () => {
      // Only remove hovered if no dropdown is open
      if (!document.body.classList.contains('has-dropdown-menu')) {
        this.headerSection.classList.remove('header-hovered');
      }
    });
  }
...
class DetailsDropdown extends HTMLDetailsElement {
  ...
  async transition(value) {
    if (value) {
      ...
      // When last dropdown closes, also clean up header-hovered if mouse left
      if (clearDropdownCount.get(DetailsDropdown) === 0) {
        const headerSection = document.querySelector('.header-section');
        const header = headerSection && headerSection.querySelector('header');
        if (header && !header.matches(':hover')) {
          headerSection.classList.remove('header-hovered');
        }
      }
