...
const { extractHubProcessedImages, extractRemoteSourceUrls } = require('./af_demo_handoff');
...
function mapHubProduct(hub, options = {}) {
  ...
  const priceHidden = forcePriceHidden || mapPriceAuthority(hub.price_authority);
  ...
  const processedImages = extractHubProcessedImages(hub, options);
  const hubProcessedMediaOnly = options.hubProcessedMediaOnly === true;
  const images = processedImages.length
    ? processedImages
    : (hubProcessedMediaOnly ? [] : extractRemoteSourceUrls(hub, options));
  ...
  return {
    ...
    descriptionHtml: hub.description_html || hub.description || '',
    tags: [...new Set(tags)],
    images,
    variants,
    metafields,
