...
async function attachRemoteProductImages(productId, imageUrls, options = {}) {
  ...
  const normalizedUrls = [...new Set((imageUrls || []).map(normalizeImageUrl).filter(Boolean))];
  ...
  const remoteSources = normalizedUrls.map((url, index) => ({
  ...
  try {
    ...
    if (!allowProxyFallback) {
      return {
        ...
        created: [],
        poll: null,
        errors: [{ stage: 'create', message: err.message, urls: normalizedUrls }],
      };
    }
    created = [];
  }
  ...
  const needsProxy = !poll.ok || failedRemote.length > 0 || created.length === 0;
  ...
  for (let index = 0; index < normalizedUrls.length; index++) {
    ...
    try {
      ...
      proxyResults.push({ url, method: 'proxy_staged_upload', mediaId: mediaNode.id });
    } catch (err) {
